...
// ─────────────────────────────────────────────────────────────
// 1. BUSCA HÍBRIDA — MySQL e ODBC em paralelo, independentes
// ─────────────────────────────────────────────────────────────
app.get("/api/funcionarios", async (req, res) => {
    const busca = (req.query.busca || "").trim();
    if (!busca) return res.json({ resultados: [], fontes: {} });

    // Executa as duas buscas em paralelo; uma falha não afeta a outra
    const [resMySQL, resODBC] = await Promise.all([
        buscarNoMySQL(busca),
        buscarNoODBC(busca)
    ]);

    // Mescla e deduplica por matrícula (MySQL tem prioridade — dados já cadastrados)
    const uniqueMap = new Map();
    [...resMySQL.resultados, ...resODBC.resultados].forEach(emp =>
        uniqueMap.set(String(emp.matricula), emp)
    );

    const resultados = Array.from(uniqueMap.values());

    // Devolve também o status de cada fonte para o frontend poder informar o usuário
    res.json({
        resultados,
        fontes: {
            mysql: { ok: resMySQL.erro === null, erro: resMySQL.erro },
            odbc:  { ok: resODBC.erro  === null, erro: resODBC.erro  }
        }
    });
});
...
app.post("/api/funcionarios/novo", async (req, res) => {
    ...
    try {
        ...
        const [existe] = await connection.execute(
            "SELECT id FROM funcionarios WHERE matricula = ?", [matricula]
        );
        if (existe.length > 0) {
